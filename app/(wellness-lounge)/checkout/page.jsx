...
const AppointmentBooking = () => {
  ...
  const getDetails = async (id) => {
    ...
    try {
      ...
      sessionList(res?.lounge_type?.id);
  ...
  const sessionList = async (id) => {
    try {
      const body = {
        lounge_type: id,
        start_date__gt: moment(new Date()).format("YYYY-MM-DD HH:mm:ss"),
      };

      const res = await Models.session.linesList(1, body);

      setState({ lines: res, loading: false }); // Set data and stop loading
    } catch (error) {
      console.log("Error:", error);
  ...
  const paymentGatewayList = async () => {
    try {
      ...
      console.log("paymentGatewayList --->", res);
  ...
  const departments = [
    {
      id: "clinical",
      name: "Ayurvedic One-on-One Counseling Lounge",
      description: "Ayurvedic One-on-One Counseling Lounge",
      price: "Starting from ₹500",
      items: [
        { id: "anaesthesiology", name: "New Session", price: "₹500" },
        { id: "andrology", name: "New Wellness Lounge", price: "₹800" },
        { id: "cardiology", name: "Navigating College Life", price: "₹700" },
      ],
    },
  ];
  ...
  const toggleAccordion = (departmentId) => {
    if (state.isOpenAccordion === departmentId) {
      setState({ isOpenAccordion: null });
    } else {
      setState({ isOpenAccordion: departmentId });
    }
  };

  const toggleSpecialtySelection = async (item) => {
    try {
      if (state.lines.includes(item?.id)) {
        setState({ selectedLine: [] });
      } else {
        let total = 0;
        const find = state.lines.find((line) => line?.id == item?.id);
        total += parseInt(find.price.replace("₹", "").replace(",", ""));
        if (state.couponAmount > 0) {
          if (parseInt(find?.price) >= parseInt(state.couponAmount)) {
            total = parseInt(find.price) - state.couponAmount;
          } else {
            total = 0;
          }
        }
  ...
  const verifyPayment = async (response) => {
    try {
      ...
      const res = await Models.session.verifyPayment();
      console.log("paymentGatewayList --->", res);
  ...
  const handleStep1Next = async () => {
    try {
      const userId = localStorage.getItem("userId");
      // console.log("✌️state.selectedRecord --->", state.selectedRecord);
      ...
      const startDate = new Date(state.selectedRecord?.start_date);

      const endDate = new Date(state.selectedRecord?.end_date);
  ...
  const removeCoupon = () => {
    setState({ couponAmount: 0, total: parseInt(state.selectedRecord?.price) });
  };
  console.log("✌️state.selectedDate --->", state.selectedDate);
  ...
  return (
    <div className="container mx-auto max-w-6xl p-4 border border-1 rounded-md">
      ...
      <p className="text-gray-600 mb-6">Select the Session from below</p>
...
export default ProtectedRoute(AppointmentBooking);
